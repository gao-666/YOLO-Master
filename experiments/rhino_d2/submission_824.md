...
| 基线/最小任务 | YOLO-Master-N P4 与冻结 DINOv2-small 对齐；真实 detection task loss 与 cosine KD loss 共同反传；固定 batch 训练 5 步。 |
| 复现命令 | `python experiments/rhino_d2/scripts/d2_p0_train_smoke.py --config experiments/rhino_d2/configs/d2_p0.yaml --offline`；专项 `pytest` 54 passed。 |
...
| 结果证据 | P0 9 项检查全部通过；KD `0.09993→0.09786`；独立 projector smoke `0.97372→0.38375`；KD 进入 total；学生/投影梯度非零；teacher frozen 且不在 optimizer；54 tests passed。 |
